= 高速インストール (Fast Install)

最短で稼働させるための手順、ヒント、コツを紹介します。

[NOTE]
====
*このページを使用するタイミング：* このモジュールは、各モジュールの説明を省き、ラボ環境全体を迅速にセットアップしたい経験豊富なユーザー向けの要約リファレンスです。このワークショップが初めての場合は、代わりに各モジュールを順に実行してください。各ステップの内容や理由について重要なコンテキストが提供されています。
====

=== 高速インストール手順

ログイン

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
{login_command}
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
git clone https://github.com/burrsutter/fantaco-redhat-one-2026
cd fantaco-redhat-one-2026/
----

このラボ環境の一部として、いくつかの重要な変数が事前に入力されています。

最も重要な環境変数を `my_env_vars.sh` に保存します。これにより、変数が失われた場合でも、単に `source $HOME/fantaco-redhat-one-2026/my_env_vars.sh` を実行するだけで復旧できます。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
cat << 'EOF' > my_env_vars.sh
export VLLM_API_URL='{litellm_api_base_url}'
export VLLM_API_TOKEN='{litellm_virtual_key}'
export LLAMA_STACK_BASE_URL=http://llamastack-distribution-vllm-service.agentic-{user}.svc:8321
export INFERENCE_MODEL=vllm/qwen3-14b
export API_KEY="not-applicable"
export EMBEDDING_MODEL=sentence-transformers/nomic-ai/nomic-embed-text-v1.5
export EMBEDDING_DIMENSION=768
export CUSTOMER_MCP_SERVER_URL=https://$(oc get routes -l app=mcp-customer -o jsonpath="{range .items[*]}{.status.ingress[0].host}{end}")/mcp
export FINANCE_MCP_SERVER_URL=https://$(oc get routes -l app=mcp-finance -o jsonpath="{range .items[*]}{.status.ingress[0].host}{end}")/mcp
export CHAT_URL=http://$(oc get routes -l app=simple-agent-chat-ui -o jsonpath="{range .items[*]}{.status.ingress[0].host}{end}")
export LANGFUSE_HOST=https://$(oc get route langfuse -o jsonpath='{.spec.host}')
EOF
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source $HOME/fantaco-redhat-one-2026/my_env_vars.sh
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
echo "VLLM_API_URL="$VLLM_API_URL
echo "VLLM_API_TOKEN="$VLLM_API_TOKEN
echo ""
echo "LLAMA_STACK_BASE_URL="$LLAMA_STACK_BASE_URL
echo "INFERENCE_MODEL="$INFERENCE_MODEL
echo "API_KEY="$API_KEY
echo ""
echo "EMBEDDING_MODEL"=$EMBEDDING_MODEL
echo "EMBEDDING_DIMENSION="$EMBEDDING_DIMENSION
echo ""
echo "CUSTOMER_MCP_SERVER_URL="$CUSTOMER_MCP_SERVER_URL
echo "FINANCE_MCP_SERVER_URL="$FINANCE_MCP_SERVER_URL
echo ""
echo "CHAT_URL"=$CHAT_URL
echo ""
echo "LANGFUSE_HOST"=$LANGFUSE_HOST
----

注：これらの一部は、手順を進めるにつれて入力されます。

[.console-output]
[source,bash,subs=attributes+]
----
https://llm-prod.apps.maas.redhatworkshops.io/v1
sk-331A-A8c_DxQiHnUPX1Ulg
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc apply -f llama-stack-scripts/llamastack-configmap.yaml
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc apply -f - <<'EOF'
apiVersion: llamastack.io/v1alpha1
kind: LlamaStackDistribution
metadata:
  annotations:
    openshift.io/display-name: llamastack-distribution-vllm
  name: llamastack-distribution-vllm
  labels:
    opendatahub.io/dashboard: 'true'
spec:
  replicas: 1
  server:
    containerSpec:
      command:
        - /bin/sh
        - '-c'
        - llama stack run /etc/llama-stack/run.yaml
      env:
        - name: VLLM_TLS_VERIFY
          value: 'false'
        - name: MILVUS_DB_PATH
          value: ~/.llama/milvus.db
        - name: FMS_ORCHESTRATOR_URL
          value: 'http://localhost'
        - name: VLLM_MAX_TOKENS
          value: '4096'
        - name: VLLM_API_URL
          value: '{litellm_api_base_url}'
        - name: VLLM_API_TOKEN
          value: '{litellm_virtual_key}'
        - name: LLAMA_STACK_CONFIG_DIR
          value: /opt/app-root/src/.llama/distributions/rh/
      name: llama-stack
      port: 8321
      resources:
        limits:
          cpu: '2'
          memory: 12Gi
        requests:
          cpu: 250m
          memory: 500Mi
    distribution:
      name: rh-dev
    userConfig:
      configMapName: llama-stack-config
EOF
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
echo "LLAMA_STACK_BASE_URL="$LLAMA_STACK_BASE_URL
echo "INFERENCE_MODEL="$INFERENCE_MODEL
echo "API_KEY="$API_KEY
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc get pods -w
----

[.console-output]
[source,bash,subs=attributes+]
----
NAME                                           READY   STATUS     RESTARTS   AGE
llamastack-distribution-vllm-9b647c5b5-xhfxs   0/1     Init:0/1   0          43s
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
./install-lls-fantaco.sh
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc get pods -w
----

[.console-output]
[source,bash,subs=attributes+]
----
NAME                                            READY   STATUS    RESTARTS   AGE
fantaco-customer-main-7fd4ddb666-4q8dh          1/1     Running   0          55s
fantaco-finance-main-75ffddb44b-t8rrg           1/1     Running   0          55s
langgraph-fastapi-569d6d554-cjnxl               1/1     Running   0          52s
llamastack-distribution-vllm-85866c49c6-k769c   1/1     Running   0          3m4s
mcp-customer-6bd8bcfc7b-kcjg5                   1/1     Running   0          53s
mcp-finance-8cc684b8d-d254r                     1/1     Running   0          53s
postgresql-customer-ff78dffdf-jrlhp             1/1     Running   0          55s
postgresql-finance-689d97894f-hks84             1/1     Running   0          55s
simple-agent-chat-ui-6d7794dc6b-5j6fs           1/1     Running   0          52s
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source my_env_vars.sh
echo $CUSTOMER_MCP_SERVER_URL
echo $FINANCE_MCP_SERVER_URL
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
echo $CHAT_URL
----

=== Langfuseのセットアップ (Langfuse Setup)

$HOME/fantaco-redhat-one-2026/ 内に .venv フォルダを保持します。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
cd $HOME/fantaco-redhat-one-2026/
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
python -m venv .venv
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source .venv/bin/activate
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
cd $HOME/fantaco-redhat-one-2026/langfuse-setup
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source ./create-langfuse-url.sh
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source ./create-secrets.sh
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
helm repo add langfuse https://langfuse.github.io/langfuse-k8s
helm repo update
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
helm install langfuse langfuse/langfuse \
    -f values-openshift-single-user.yaml \
    --set langfuse.nextauth.url=$LANGFUSE_HOST
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc get pods -w
----

[.console-output]
[source,bash,subs=attributes+]
----
NAME                                            READY   STATUS    RESTARTS      AGE
fantaco-customer-main-7fd4ddb666-c26fv          1/1     Running   0             55m
fantaco-finance-main-75ffddb44b-frlxb           1/1     Running   0             55m
langfuse-clickhouse-shard0-0                    1/1     Running   0             23m
langfuse-postgresql-0                           1/1     Running   0             23m
langfuse-redis-primary-0                        1/1     Running   0             23m
langfuse-s3-5fb6c8f845-s9shp                    1/1     Running   0             23m
langfuse-web-67f4c9849-rmkxt                    1/1     Running   2 (23m ago)   23m
langfuse-worker-69d997664c-bdr8f                1/1     Running   0             23m
langfuse-zookeeper-0                            1/1     Running   0             23m
langgraph-fastapi-569d6d554-65j95               1/1     Running   0             55m
llamastack-distribution-vllm-78ccd8fcbf-tp2ch   1/1     Running   0             59m
mcp-customer-6bd8bcfc7b-fmmt2                   1/1     Running   0             55m
mcp-finance-8cc684b8d-bln8l                     1/1     Running   0             55m
postgresql-customer-ff78dffdf-69p5n             1/1     Running   0             55m
postgresql-finance-689d97894f-jlmm8             1/1     Running   0             55m
simple-agent-chat-ui-6d7794dc6b-4cndl           1/1     Running   0             55m
----

langfuse-web は CrashLoop になりやすいため、トラブルシューティングのセクションを参照してください。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
echo $LANGFUSE_HOST
----

[.console-output]
[source,bash,subs=attributes+]
----
https://langfuse-agentic-user1.apps.cluster-m7dw5.dynamic.redhatworkshops.io
----

LangfuseのGUIを使用してAPIキーを作成します。

値を `keys.txt` に記述します。これは、ブラウザの更新、タイムアウト、クラスターの停止/スリープなどによって環境変数が失われた場合に役立ちます。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
vi keys.txt
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source $HOME/fantaco-redhat-one-2026/langfuse-setup/convert-keys-txt-env-vars.sh
----

必要なLangfuse環境変数が設定されていることを確認します。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
echo $LANGFUSE_SECRET_KEY
echo $LANGFUSE_PUBLIC_KEY
echo $LANGFUSE_HOST  
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
cd ~/fantaco-redhat-one-2026/langfuse-setup/langgraph-agent/backend
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
pip install -r requirements.txt
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
python 6-langgraph-langfuse-fastapi-chatbot.py
----

これはさまざまな理由で失敗する可能性があります。起動ログを確認して、すべての環境変数が正しく設定されているか確認してください。

ターミナル 2

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source $HOME/fantaco-redhat-one-2026/langfuse-setup/convert-keys-txt-env-vars.sh
echo $LANGFUSE_SECRET_KEY
echo $LANGFUSE_PUBLIC_KEY
echo $LANGFUSE_HOST  
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
curl -sS -X POST http://localhost:8002/chat \
  -H "Content-Type: application/json" \
  -d '{
    "message": "what are Fran Wilson'\''s orders?",
    "user_id": "test-user",
    "session_id": "test-session-123"
}' | jq -r '.reply'
----

=== 評価 (Evals)

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
curl -sS -X POST http://localhost:8002/evaluate \
  -H "Content-Type: application/json" \
  -d '{
    "run_name": "after-prompt-update",
    "sync_dataset": true,
    "record_to_langfuse": true
  }' | jq .
----

=== フィードバック (Feedback)

Routeは、showroomネームスペース内でのクラスター作成時に事前に作成されています。このスクリプトはパターンに基づいてそれを算出するだけです。
フィードバックを残すには、チャットボットのGUIにアクセスする必要があります。

Showroomユーザーの場合：

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
$HOME/fantaco-redhat-one-2026/langfuse-setup/get-agent-route-as-student.sh
----

または

Workbenchユーザーの場合：

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc apply -f workbench-service-route.yaml
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
echo "https://$(oc get route chatbot-8002 -o jsonpath='{.spec.host}')"
----

image::langfuse_feedback_1.png[]

image::langfuse_feedback_2.png[]

正しい環境変数が設定されていることを確認してください。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source $HOME/fantaco-redhat-one-2026/langfuse-setup/convert-keys-txt-env-vars.sh
----

トレースの一覧表示

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
source $HOME/fantaco-redhat-one-2026/langfuse-setup/list-traces.sh
----

最後のトレースのフィードバックはどうなっていますか？

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
$HOME/fantaco-redhat-one-2026/langfuse-setup/get-trace-feedback.sh
----

image::langfuse_feedback_3.png[]

== トラブルシューティング (Troubleshooting)

==== OpenShift AI (旧称 RHODS)

LlamaStackはまだデベロッパープレビュー段階であり、後方互換性がないことがよくあります。

クラスター管理者として

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc get csv -A -o custom-columns=NAME:.metadata.name,VERSION:.spec.version | grep rhods | head -1
----

[.console-output]
[source,bash,subs=attributes+]
----
rhods-operator.3.2.0                      3.2.0
----

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc get llamastackdistribution -A -o custom-columns=NAME:.metadata.name,OPERATOR_VERSION:.status.version.operatorVersion,LLAMASTACK_VERSION:.status.version.llamaStackServerVersion
----

[.console-output]
[source,bash,subs=attributes+]
----
NAME                           OPERATOR_VERSION   LLAMASTACK_VERSION
llamastack-distribution-vllm   "0.4.0"            0.3.5.1+rhai0
----

LLSポッドを監視する

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc adm top pod -l app=llama-stack
----

[.console-output]
[source,bash,subs=attributes+]
----
NAME                                            CPU(cores)   MEMORY(bytes)
llamastack-distribution-vllm-6954d965d8-m29kr   2m           461Mi
----

= Langfuse: トレース、評価（Evals）、フィードバック

エージェントはアプリケーションであり、大規模に実行するための適切なインフラストラクチャを必要とします。レプリカ間のロードバランシングやフェイルオーバー、ログとメトリクスの取得、自動再起動といった重要な機能の多くは、OpenShiftの一部として提供されています。

しかし、エージェントには以下のような追加のインフラストラクチャサービスを必要とする独自の側面があります。

* トレース（Traces）
* 評価（Evals）
* フィードバック（Feedback）

これらの機能を実現するためのオープンソースソリューションが *Langfuse* です。

[TIP]
====
*なぜエージェントには専用のオブザーバビリティが必要なのでしょうか？* 従来のアプリケーションモニタリングでは、リクエストのレイテンシーやエラー率を追跡します。エージェントのオブザーバビリティでは、**推論チェーン（reasoning chain）**も取得する必要があります。具体的には、どのツールが呼び出されたか、どのようなデータが返されたか、モデルがどのように回答を合成したか、そしてユーザーがそれを有益だと感じたか、といった点です。Langfuseは、OpenShiftがすでに提供しているプラットフォームレベルのモニタリングの上に、これらAI特有のオブザーバビリティレイヤーを提供します。
====

トレースは、デバッグ、本番環境のサポート、およびエンドユーザーの意図の初期取得とそれに続く評価（evals）の作成において非常に重要です。
フィードバックは、時間をかけてエージェントの品質を段階的に向上させるために不可欠です。
...
LangGraphクライアント（およびほとんどのクライアント）は、たとえLlama Stackを介してプライベートにホストされたモデルを使用しており適用外であっても、API_KEYが設定されていることを好みます。
...
以下のレスポンスは、Python venvを作成する必要があることを示しています。
...
次に、エージェントのバックエンドディレクトリに移動します。ここには、Langfuseトレース用に計装された別のLangGraphベースのエージェントがあります。
...
== Langfuseのセットアップ (Langfuse Setup)
...
workerポッドまたはwebポッドのログを表示できます。Kubernetesの結果整合性が機能するまでの間、数回再起動が発生することがあります。
...
名前、メールアドレス、パスワードを入力し、**サインアップ (Sign up)** をクリックします。

image::langfuse-sign-up-1.png[]

**新規組織 (New Organization)** をクリックします。

image::langfuse-sign-up-2.png[]

組織名に **my-agent-org** と入力し、**作成 (Create)** をクリックします。
...
プロジェクト名に **my-agent-project** と入力し、**作成 (Create)** をクリックします。
...
**APIキー (API Keys)** をクリックします。

image::langfuse-sign-up-7.png[]

**+ 新規APIキーを作成 (+ Create new API Keys)** をクリックします。

image::langfuse-sign-up-8.png[]

キー名に **my-key** と入力し、**APIキーを作成 (Create API keys)** をクリックします。
...
値を `keys.txt` に記述します。これは、ブラウザの更新、タイムアウト、クラスターの停止/スリープなどによって環境変数が失われた場合に役立ちます。
...
[NOTE]
====
*トレースは何を示していますか？* 各トレースは、リクエストの完全なライフサイクルを取得します。ユーザーの入力、すべてのLLM推論呼び出し、すべてのツール実行（顧客および財務サービスへのMCP呼び出しを含む）、ツールのレスポンス、および最終的に生成された回答が含まれます。これは、エージェントがなぜ誤った回答をしたのかをデバッグする際に非常に役立ちます。どのツールが予期しないデータを返したのか、あるいはモデルの推論のどこに誤りがあったのかを正確に把握できます。
====

== 評価（Evals）

バックエンドは、Langfuse評価を実行するための呼び出しも提供します。`6-langgraph-langfuse-fastapi-chatbot.py` を開くと、`@app.post("/evaluate", response_model=EvaluationResponse)` を介して定義されたエンドポイントが見つかります。

これは内部でLangfuse APIと対話して評価を実行します。

[source, python]
----
  # 評価実行の開始: {request.run_name or '自動生成'}
...
[NOTE]
====
*キーワードベースの評価（Keyword-based evals）:* これらの評価はシンプルですが効果的なアプローチを採用しています。エージェントの回答に特定の期待されるキーワードが含まれているかどうかをチェックします。例えば、「Thomas Hardy」のテストでは「Around the Horn」と「Sales Representative」を期待します。スコア1.0は、すべてのキーワードが一致したことを意味します。これは、LLM-as-judge（Llama Stack Evalsモジュールで確認したもの）よりも軽量ですが、エージェントがツールから正しい事実データを返していることを検証するのに適しています。
====

== フィードバック (Feedback)

フィードバックシステムはGUIに紐づいた親指アップ/ダウン（Thumbs-up/down）機能で、その情報をスコアとしてLangfuseに追加します。

フィードバックシステムを使用するには、チャットボットUIにアクセスする必要があります。

Showroomターミナルを使用している場合は、以下のスクリプトを実行して、事前に作成されたルート（Route）を確認できます。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
$HOME/fantaco-redhat-one-2026/langfuse-setup/get-agent-route-as-student.sh
----

[.console-output]
[source,bash,subs=attributes+]
----
CHAT_TRACE_URL=https://chatbot-8002-showroom-qk9xm-1-user1.apps.ocp.qk9xm.sandbox2661.opentlc.com/
----

**または**

Workbench Code Server IDEターミナルを使用している場合**のみ**、以下のコマンドでサービスとルートを追加できます。

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
oc apply -f $HOME/fantaco-redhat-one-2026/langfuse-setup/workbench-service-route.yaml
echo "CHAT_TRACE_URL=https://$(oc get route chatbot-8002 -o jsonpath='{.spec.host}')"
----

[.console-output]
[source,bash,subs=attributes+]
----
CHAT_TRACE_URL=https://chatbot-8002-agentic-user1.apps.ocp.82j4f.sandbox628.opentlc.com
----

チャットボットでは、質問をしたり、親指アップ/ダウンやコメントを使用してフィードバックを提供したりできます。

image::langfuse_feedback_1.png[]

image::langfuse_feedback_2.png[]
...
トレースIDを使用して、特定のスコアとコメントを取得します。トレースIDを指定しない場合は、デフォルトで最後のトレースが取得されます。
...
[source,subs=attributes+]
----
No trace ID provided, fetching latest trace...
...
**get-trace-feedback.sh** に特定のトレースIDを指定して実行することも可能です。

== まとめ (Summary)

このモジュールでは以下を行いました。

* AIエージェントのオブザーバビリティプラットフォームとして *Langfuse* をデプロイしました。
* ユーザー入力からツールの呼び出し、最終的なレスポンスに至るまで、各エージェントのやり取りの完全な推論チェーンを示す **トレース（traces）** を取得しました。
* エージェントに対して **キーワードベースの評価（evals）** を実行し、正しい顧客データと注文データが返されていることを検証しました。
* 特定のトレースに紐づいたエンドユーザーの満足度を取得するために、**フィードバックシステム**（コメント付きの親指アップ/ダウン）を使用しました。

これら3つの機能（トレース、評価、フィードバック）は継続的な改善ループを形成します。トレースはデバッグに役立ち、評価はデグレードの防止に役立ち、フィードバックは次に何を改善すべきかの発見に役立ちます。
